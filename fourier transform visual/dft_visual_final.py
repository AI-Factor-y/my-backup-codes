...
def draw_epicycles(x,y,rotation,fourier):
	...
	for i in range(len(fourier)):
		...
		try:
			pass
			# pygame.draw.circle(gameDisplay,black,(int(prevx),int(prevy)),int(radius),1)
		except:
			pass

		pygame.draw.line(gameDisplay,black,(int(prevx),int(prevy)),(int(x),int(y)))
...
while run:
	...
	pygame.draw.line(gameDisplay,black,(vy[0],vy[1]),(v[0],v[1]))
	...
	for i in range(len(path)):
		...
		pygame.draw.line(gameDisplay,red,(int(prevx),int(prevy)),(int(path[i][0]),int(path[i][1])),2)
		prevx=path[i][0]
		prevy=path[i][1]
