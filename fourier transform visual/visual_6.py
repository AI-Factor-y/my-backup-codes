...
def get_draw_cord():
	...
	while draw_Run:
		...
		if pygame.mouse.get_pressed()[0]:
			x.append(pygame.mouse.get_pos()[0]-500)
			y.append(pygame.mouse.get_pos()[1]-300)

		for i in range(len(x)):
			pygame.draw.circle(gameDisplay,white,(x[i]+500,y[i]+300),5)
...
while run:
	...
	vx=draw_epicycles(width/2+100,100,0,fourierX)
	vy=draw_epicycles(100,height/2+100,math.pi/2,fourierY)
	v=(vx[0],vy[1])
	...
	for i in range(len(path)):
		...
		pygame.draw.line(gameDisplay,white,(int(prevx),int(prevy)),(int(path[i][0]),int(path[i][1])))
		prevx=path[i][0]
		prevy=path[i][1]
